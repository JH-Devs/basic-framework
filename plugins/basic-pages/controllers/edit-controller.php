...
if(!empty($row))
{
	...
	if($csrf && $files_ok && $page->validate_update($postdata))
	{
		if(user_can('edit_page'))
		{
			...
			$cm->disabled_all($page_id); // Vypněte všechny kategorie pro tuto stránku

			if (!empty($postdata['category'])) {
				// Uložte nové kategorie
				$cm->save_new($page_id, $postdata['category']);
			} else {
				// Pokud nejsou žádné kategorie, nechejte je jako zakázané
				$cm->disabled_all($page_id); // Nebo tuto část vynechejte, pokud je disable_all již voláno
			}
