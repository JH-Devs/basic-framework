...
add_action('controller', function() {
    ...
    if (URL(1) == $vars['plugin_route'] && $req->posted()) {
        ...
        $category_roles_map = new \Categories\Category;
...
add_action('basic-admin_main_content', function() {
    ...
    if (URL(1) == $vars['plugin_route']) {
        ...
        if (URL(2) == 'add') {
            $category_role = new \Categories\Category;
        ...
        if (URL(2) == 'view') {
            ...
            if(!empty($_GET['find'])) {
                $find = '%' . trim($_GET['find']) . '%';
                $categories_table = $tables = $vars['tables']['categories_table'];

                $query = "select * from $categories_table where (category like :find || slug like :find) limit $limit offset $offset";
...
add_filter('after_query',function($data){
	...
	{
		$role_map = new \Categories\Category;
