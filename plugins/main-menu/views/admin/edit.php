...
<form onsubmit="submit_form(event)" method="post" enctype="multipart/form-data">
    ...
    <?=csrf()?>
        ...
        <label class="text-center  mb-3">
        Mega image: <br>
            <img src="<?=get_image($row->mega_image)?>" class="img-thumbnail" style="cursor:pointer;width:100%;max-width:200px;height:200px;object-fit:cover;"/>
            <input onchange="display_image(event)" type="file" name="mega_image" class="d-none">

            <?php if (!empty($errors['mega_image'])): ?>
                <small class="text-danger px-2"><?= $errors['mega_image'] ?></small>
            <?php endif ?>

            <div class="mt-3">
            <input type="checkbox" value="1" name="remove_mega_image"> <small class="text-red"> Remove image</small>
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            <select class="form-select" name="permission">
                <option <?= old_select('', '') ?> value="">
                    <?= esc(ucfirst(str_replace('_', ' ', ''))) ?>
                </option>
        ...
        <div class="mb-2 col-md-6">
            <label for="is_mega" class="form-label">Is Mega</label>
            <select class="form-select" name="is_mega">
                <option <?= old_select('is_mega', '1', $row->is_mega) ?> value="1"><?= esc('Yes') ?></option>
                <option <?= old_select('is_mega', '0', $row->is_mega) ?>  value="0"><?= esc('No') ?></option>
            </select>
            <?php if (!empty($errors['is_mega'])): ?>
                <small class="text-danger px-2"><?= $errors['is_mega'] ?></small>
            <?php endif ?>
        </div>
