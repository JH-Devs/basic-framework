...
class Model extends Database
{
    ...
    public function delete_all(array $ids)
    {
    if (!empty($ids)) {
        // Vytvoříme řetězec s placeholders pro dotaz
        $placeholders = implode(',', array_fill(0, count($ids), '?'));

        // Vytvoříme dotaz pro smazání více záznamů
        $query = "DELETE FROM $this->table WHERE $this->primary_key IN ($placeholders)";
