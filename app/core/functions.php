<?php
function set_value(mixed $key, mixed $value = '') : bool
{
    global $USER_DATA;
...
/** rozdělí řetězec dotazu v adrese URL */
...
/** načte pluginy i s jejich informacemi a nastavení jak se mají zobrazovat a kde */
                ...
                foreach ((array)$json->dependencies as $plugin_id => $version) {
                    if($plugin_data = plugin_exists($plugin_id)) {
                        ...
                        if ($existing_version < $required_version) {
                            ...
                            dd("Missing plugin dependency: " . $plugin_id . " version: ".$version.", Request by plugin: " . $json->id);
                        }
...
/** funkce přidání akce */
/** $priority složí k řazení podle důležitosti nebo posloupnosti  nebo jak to nazvat, můžu dát číslo až do 100.*/
...
function dd($data)
{
    echo "<div style='margin:10px;background-color:#444;color:#FFFFFF;padding:5px 10px;'>";
    echo "<pre>";
    print_r($data);
    echo "</pre>";
    echo "</div>";  
...
{
    // Získání cesty ke složce s pluginy
    $plugin_dir = get_plugin_dir(__FILE__); // __FILE__ odkazuje na aktuální soubor

    $plugins = [];

    foreach (scandir($plugin_dir) as $plugin_folder) {
        if ($plugin_folder === '.' || $plugin_folder === '..') continue;

        $config_file = $plugin_dir . '/' . $plugin_folder . '/config.json';
        if (file_exists($config_file)) {
            $plugin_data = json_decode(file_get_contents($config_file));
            $plugins[] = $plugin_data;
        }
    }
...
// functions for connecting plugins such as categories to pages
