...
class Thunder 
{
    public function make(array $args)
    {
        ...
        if ($action == 'make:migration') {
           ...
           if (!file_exists($folder))
                $this->message("Plugins folder not found", true); 
           ...
           $filename = $migration_folder . date("d-m-Y_His_") . $table_name . '.php';
        ...
        if ($action == 'make:model') {
           ...
           $filename = $model_folder . $class_name . '.php';
            ...
            $this->message("Unknow Terminal " . $action);
        } 
    }
    ...
    public function migrate(array $args) 
    {
        ...
        if ($action == 'migrate' || $action == 'migrate:rollback') {
            ...
            foreach ($folders as $folder) {
                ...
                if (!empty($file_name)) {
                    ...
                    $this->message("Migration file:" . $file . "\n\r");
                    ...
                    if (!empty($files)) {
                        ...
                        $this->message("No migration files found in specifield folder ");
                    }
                }
            }
        } else
     ...
     public function help(string|array $version) : void
     {
        ...
        echo "
        ThunderPHP v$version Terminal tool

        Database
        migrate             Locates and runs a migration from the specifield plugin folder.
        migrate:refresh     Does a rollback followed by a miigration.
