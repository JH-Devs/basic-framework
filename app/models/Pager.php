...
class Pager
{
    ...
    // tato funkce se bude volat tam, kde potřebuju stránkování
    public function display()
    {
        ?>
        <nav aria-label="Page navigation example" style="width:80%; background:none;">
            <ul class="pagination ">
                <li class="page-item"><a class="page-link" href="<?=$this->links['first']?>">First</a></li>
                <?php for($x = $this->start; $x < $this->end; $x++) : ?>
                <li class="page-item <?= $x == $this->page_number ? 'active': ''?>"><a class="page-link" href="<?=preg_replace("/page=[0-9]+/", "page=" .$x  , $this->links['current'])?>"><?= $x ?></a></li>
                <?php endfor; ?>
